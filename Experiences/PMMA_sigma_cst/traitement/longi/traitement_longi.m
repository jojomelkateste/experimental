...
%namebase = "C:\Users\Utilisateur\Desktop\Experimental\experimental_reprise\MesManip\PMMA_reflexion_MHZ_sigmat_cst\longitudinal\";
namebase = "C:\Users\Utilisateur\Desktop\Experimental\Experiences\PMMA_sigma_cst\longitudinal\";
name_c1 = namebase+"longi_fc_1p5.mat";%0.5 ok
name_c2 = namebase+"longi_fc_1p5.mat"; %ù si je n'ai pas deux qualibre je met deux fois le meme calibre ca change rien
fc = 1.5e6;
...
MinPeakDistance =  500; % pas en indices pour la distence minimum pour comencer
MinPeakHeight1  = 2e-3;
...
vg_temp2vol = mean([vs_e,vs_r]);
err_vg = std([vs_e,vs_r]);
Q = 2*pi*fc/2/vg_temp2vol/ki_r;
Q_err = sqrt((err_vg/vg_temp2vol)^2+(ki_err_r.normr)^2) * Q;

data2save.temps2vol.vg_temp2vol = vg_temp2vol;
data2save.temps2vol.err_vg = err_vg;
...
[pulse_e,pulse_r] = data_and_window2pulses(output,window_str,nb_sat_e,nb_sat_r, plot_pulses);

% detectons la vrai fc:
TF_pulse1 = abs(fftshift(fft(pulse_r{1})));
TF_pulse1(output.freq<0)=0;
[mfc,ind_fc] = max(TF_pulse1);
% figure;
% plot(output.freq,TF_pulse1);
% hold on;
% scatter(output.freq(ind_fc),mfc);
real_fc = output.freq(ind_fc);
data2save.real_fc= real_fc;
disp("Vrai fc trouve : "+real_fc)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Choix du delta_f %%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
delta_f = abs(fc-real_fc)+0.1e6;
...
% Figure a +-0.1Mhz de la freq centralpour l exemple 
[data] = pulses2data(output.freq,pulse_r{1},pulse_r{2},e, ...
...
hold on
errorbar(fc, vg_temp2vol, err_vg, err_vg, 0, 0, ...
    'LineStyle','none', ...
    'Marker','+', ...
    'MarkerSize',12, ...
    'LineWidth',2);
...
%% LA ON A LE CHOIX ENTRE CENTRER A LA FREQUENCE NOMINAL OU CENTRER A LA VRAI FREQUENCE CENTRALE
%  Je choisis pour le moment de centrer a la vrai frequence centrale mais
%  d'aller jusqu'à la frequence fc -0.1e6
[data_array_e] = map_pulses2data(output.freq,pulse_e,e, ...
    "tronq",true,"delta_f",delta_f,"fc",real_fc);
...
%% ameliorationpossible directement la liste des vg etc
...
switch reponse
    case 'Oui'
        ...
        name = "window_str_"+window_str+"_";
        name = name+"_fc_.mat";
        uisave('data2save', name); 
