function [output] = signal2pics_2calibres(name_c1,name_c2,nb_sat_e,nb_sat_r,MinPeakDistance,MinPeakHeight1,MinPeakHeight2)
    ...
    arguments (Input)
        ...
        MinPeakDistance =  1e-5 % pour trouver les pics distence minimum qui les sépare
        MinPeakHeight1  = 0.004
        MinPeakHeight2  = 0.004
    end
    disp("Attention MinPeakDistance pas utilisé pour le moment")
    % Lecture des datas
    ...
    % [pic_rc1,pic_time_rc1]  = findpeaks(envelope_r_c1,temps,'MinPeakHeight', MinPeakHeight2);%,'MinPeakDistance', MinPeakDistance);
    [pic_ec1,id_pic_ec1]  = findpeaks(envelope_e_c1,'MinPeakHeight', MinPeakHeight1);%,'MinPeakDistance', MinPeakDistance);
    [pic_rc1,id_pic_rc1]  = findpeaks(envelope_r_c1,'MinPeakHeight', MinPeakHeight2);%,'MinPeakDistance', MinPeakDistance);

    pic_time_ec1(id_pic_ec1)
    pic_time_rc1(id_pic_rc1)
    ...
    % [pic_rc2,pic_time_rc2]  = findpeaks(envelope_r_c2,temps,'MinPeakHeight', MinPeakHeight2);%,'MinPeakDistance', MinPeakDistance);
    [pic_ec2,id_pic_ec2]  = findpeaks(envelope_e_c2,'MinPeakHeight', MinPeakHeight1);%,'MinPeakDistance', MinPeakDistance);
    [pic_rc2,id_pic_rc2]  = findpeaks(envelope_r_c2,'MinPeakHeight', MinPeakHeight2);%,'MinPeakDistance', MinPeakDistance);
    
    pic_time_ec2 = temps(id_pic_ec2);
    pic_time_rc2 = temps(id_pic_rc2);
    ...
    pic_r = [];
    for i = 1:nb_sat_e
        pic_r = [pic_r;pic_rc1(i)];
    end
    pic_r = [pic_r;pic_rc2];
    
    output.pics.pic_e = pic_e;
    output.pics.pic_e_time = pic_e_time;
    output.pics.pic_r = pic_r;
