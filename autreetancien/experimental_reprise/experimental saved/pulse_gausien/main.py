...
#%% les parametre du pulse + aquisition 
read_signal = True #si on veut maniper True 
sauvegarde = False
sleep_time = 6

Fs_gen = 1e7;             # Frequence dechantillonage du generateur de fonctions
Fs_osc = Fs_gen*10;         # Frequence dechantillonage de loscillo
N_repet = 15;#500             # Nombre de repetitions du signal (a verifier experimentalement !)
F_repet = 1000;              # Frequence de repetition 
amplitudeGen = 12;  # amplitude de la tension du générateur max 12 V 
amplitudeScope1 = 0.1 # amplitude de la tension de l'oscillo 0.1
amplitudeScope2 = 10; # amplitude de la tension de l'oscillo 0.1
NombreOscParPulse = 15; #15
Freq_centrale = 100e3;        # Frequence centrale de la gaussienne
...
param = {
    ...
    'N_repet': N_repet,
    'F_repet': F_repet,
    'amplitudeGen': amplitudeGen,
    'amplitudeScope1': amplitudeScope1,
    'amplitudeScope2': amplitudeScope2,
    'NombreOscParPulse': NombreOscParPulse,
    'Freq_centrale': Freq_centrale
}
...
# Si on veut lire le signal 
if read_signal:
    se,sr  = send_g_pulse(param,sleep_time=sleep_time); # signal envoye non répété et sigan recu 
    ...
    #si on veux importer des data
    with open('../saved_data/data_bis.pkl', 'rb') as f:
...
plt.plot(freqs,abs(fft_vals) )
xlabel("f [Hz]")
# plt.plot(log10(abs(freqs)),log10(abs(fft_vals)) )
plt.show()
...
# #
# #
#%% Zone de sauvegarde des data

# commentaire = input("Comentaire pour sauvegarder? ")
...
name = str(date.year)+'.'+ str(date.month)+'.' + str(date.day ) + '_' + heure +'.' + minute +'.' + seconde
path = '../saved_data/' + name + '.pkl'


if sauvegarde >0:
    data = {'param': param, 'Ch1':Ch1,'Ch2' :Ch2, 'se':se,'t_osc':t_osc}
